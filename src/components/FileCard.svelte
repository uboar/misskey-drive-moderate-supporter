...
<div
  class="card card-bordered card-compact {file.isSensitive
    ? 'border-red-500'
    : ''} {file.checked ? 'bg-red-100' : 'bg-base-200'}"
>
  <div class="card-body">
    <div class="grid grid-cols-2">
      <div class="font-bold">
        {index + 1}<span class="text-xs text-red-600"
          >{file.isSensitive ? "（センシティブ）" : ""}</span
        >
      </div>
    ...
    <a href={file.url} target="_blank">
      {#if file.type.startsWith("image")}
        <img
          loading="lazy"
          class="media object-contain bg-white"
          src={file.thumbnailUrl}
          alt={file.url}
          style="--size: {$mediaWidth}px"
        />
    ...
    <div
      class="text-sm font-bold truncate title"
      style="--size: {$mediaWidth}px"
    >
      <p>{file.name}</p>
    </div>
    <div class="text-sm">
      <a
        href={`https://${$serverUrl}/admin/file/${file.id}`}
        tabindex="-1"
        target="_blank"
        class="link">ID : {file.id}</a
      >
    ...
    <a
      ...
      tabindex="-1"
      href={`https://${$serverUrl}/@${file.user.username}`}
      target="_blank"
    >
      @{file.user.username}
    </a>
