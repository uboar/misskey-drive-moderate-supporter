<script lang="ts">
  ...
  const gen = () => {
    ...
    generatedText += `チェックを開始したファイルの作成時刻 ： ${new Date(
      Date.parse(files[0].createdAt),
    ).toLocaleString()}\n`;
    generatedText += `チェックを終了したファイルのID ： ${
      files[files.length - 1].id
    }  <https://${$serverUrl}/admin/file/${files[files.length - 1].id}>\n`;
    generatedText += `チェックを終了したファイルの作成時刻 ： ${new Date(
      Date.parse(files[files.length - 1].createdAt),
    ).toLocaleString()}\n\n`;
    ...
    generatedText += `または <https://uboar.github.io/misskey-drive-moderate-supporter?sinceId=${
      files[files.length - 1].id
    }> からチェック出来ます。`;
  };
...
<div class="modal">
  <div class="modal-box lg:w-[800px]">
    <h2>報告用テンプレート文書作成</h2>
    <textarea
      class="textarea w-full my-2 textarea-bordered"
    ...
    <div class="flex flex-wrap w-full justify-between gap-4">
      ...
      <button class="btn btn-warning" on:click={genFlag}
        >生成(フラグ概要)</button
      >
      <button class="btn btn-secondary btn-wide" on:click={copyToClipboard}
        >クリップボードにコピー</button
      >
