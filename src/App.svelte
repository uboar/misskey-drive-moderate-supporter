...
<main>
  <div class="sticky z-20 top-0 border-b-2 w-full bg-slate-300">
    {#if !collapsed}
      <div class="md:p-4">
        <span class="font-bold text-3xl hidden lg:block"
          >Misskey Drive Moderate Supporter <span class="text-sm link"
            ><a
              href="https://github.com/uboar/misskey-drive-moderate-supporter/blob/main/README.md"
              target="_blank">使い方</a
            ></span
          ></span
        >
        ...
        <div class="flex flex-wrap md:gap-4 gap-2 items-center">
          <div>
            ...
            <input
              ...
              bind:value={sinceId}
              type="text"
              class="input input-xs md:input-md input-bordered w-32"
            />
          </div>
          <div>
          ...
          <button
            class="btn btn-sm md:btn-md md:btn-wide btn-primary"
            on:click={getFiles}
            >{files.length > 0
              ? "これより新しいファイルを取得"
              : "ファイルの取得"}</button
          ...
          {#if files.length > 0}
            <button
              class="btn btn-sm md:btn-md md:btn-wide btn-accent"
              on:click={getOldFiles}>これより古いファイルを取得</button
            >
          {/if}
          {#if files.length > 0}
            <button
              class="btn btn-sm md:btn-md btn-outline btn-primary"
              on:click={() => {
                files = [];
                getFiles();
              }}>バッファを削除して再取得</button
    ...
    {:else}
      <div class="flex">
        <button
          class="btn btn-sm btn-wide max-w-[50%] btn-primary"
          on:click={getFiles}
          >{files.length > 0
            ? "これより新しいファイルを取得"
            : "ファイルの取得"}</button
        >
        <button
          class="btn btn-info btn-sm max-w-[50%] btn-wide"
          on:click={() => {
            collapsed = false;
          }}>開く</button
        >
